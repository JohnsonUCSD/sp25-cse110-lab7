describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++){
      ...
      const itemValue = prodItemsData[i];
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const buttonProperty = await button.getProperty('innerText');
    const buttonText = await buttonProperty.jsonValue();
...
// Changed sloMo in jest-puppeteer.config.js from 25 to 15 for the tests to be able to run under 10000ms for each test